...
class MirrorAndAddLabels(MapTransform):
    def __init__(self, keys, class1=1, class2=2):
        super().__init__(keys)
        self.class1 = class1
        self.class2 = class2
    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            ...
            mask1 = (label == self.class1).astype(np.uint8)
            mask2 = (label == self.class2).astype(np.uint8)
            ...
            label = ((mask1 + mirrored_mask2) > 0) * self.class1
            label += ((mask2 + mirrored_mask1) > 0) * self.class2
