...
def get_LISA_file_list(args, test=False) -> List[List, List, List]:

    train_images = sorted(glob.glob(os.path.join(args.input_dir, 'Low Field Images', "*.nii.gz")))

    if args.target == 'hippocampus':
        train_labels = sorted(glob.glob(os.path.join(args.input_dir, "Subtask 2a - Hippocampus Segmentations", "*.nii.gz")))
        train_images = sorted(glob.glob(os.path.join(args.input_dir, "*.nii.gz")))

        train_labels = train_images
    elif args.target == 'basal_ganglia':
        train_labels = sorted(glob.glob(os.path.join(args.input_dir, "Subtask 2b - Basal Ganglia Segmentations", "*.nii.gz")))
    elif args.target == 'all_structures':
        train_labels = sorted(glob.glob(os.path.join(args.input_dir, "all_labels", "*.nii.gz")))
    ...
    if args.target == 'validation':
        # Combine and remove duplicates based on a unique key (e.g., "id")
        combined_data = train_data + val_data
        seen_ids = set()
        val_data = []
        for item in combined_data:
            if item["image"] not in seen_ids:
                val_data.append(item)
                seen_ids.add(item["image"])
        train_data = [val_data[0]]
    #val_data = [el for el in val_data if '0002' in el['image']]
...
def get_data(args, test=False):
    ...
    if args.dataset == "TriALS":
        ...
        train_data, val_data, test_data = get_LISA_file_list(args, test=test)
...
def get_train_loader(args, pre_transforms_train):
    ...
    train_ds = Dataset(train_data, pre_transforms_train)
...
def get_val_loader(args, pre_transforms_val):
    ...
    val_ds = Dataset(val_data, pre_transforms_val)
